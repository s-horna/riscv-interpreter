...
void vm_run_goto(virtualmachine* vm) {
    ...
    void* dispatch_table[128] = {&&illegal};
    dispatch_table[0b0110011] = &&r_inst;
    dispatch_table[0b0010011] = &&i_inst;
    dispatch_table[0b0000011] = &&load_inst;
    dispatch_table[0b0100011] = &&s_inst;
    dispatch_table[0b1100011] = &&b_inst;
    dispatch_table[0b1101111] = &&jal_inst;
    dispatch_table[0b1100111] = &&jalr_inst;
    dispatch_table[0b0110111] = &&lui_inst;
    dispatch_table[0b0010111] = &&auipc_inst;
    dispatch_table[0b1110011] = &&ebreak_inst;

    uint32_t instruction = vm->program[vm->pc / 4];
    goto *dispatch_table[instruction & 0b1111111];

r_inst:
    exec_R_inst(vm, instruction);
    vm->pc += 4;
    if (vm->running && vm->pc < vm->program_size) {
        instruction = vm->program[vm->pc / 4];
        goto *dispatch_table[instruction & 0b1111111];
    } else { goto end; }
i_inst:
